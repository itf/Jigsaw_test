...
function findBestNeighbor(area: ProductionArea, allAreas: ProductionArea[], currentIndex: number): number {
  ...
  if (sourcePath && sourcePath.length > 0) {
    for (let i = 0; i < steps; i++) {
      const pt = sourcePath.length > 0 ? sourcePath.getPointAt(sourcePath.length * (i / steps)) : null;
      const normal = sourcePath.length > 0 ? sourcePath.getNormalAt(sourcePath.length * (i / steps)) : null;
