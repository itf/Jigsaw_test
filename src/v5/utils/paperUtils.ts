...
export function getPointOnBoundary(boundary: paper.PathItem, t: number, pathIndex: number = 0): paper.Point {
  if (boundary instanceof paper.Path) {
    if (boundary.length === 0) return new paper.Point(0, 0);
    const pt = boundary.getPointAt(boundary.length * t);
    ...
    if (path.length === 0) return new paper.Point(0, 0);
...
export function getNormalOnBoundary(boundary: paper.PathItem, t: number, pathIndex: number = 0): paper.Point {
  if (boundary instanceof paper.Path) {
    if (boundary.length === 0) return new paper.Point(0, 1);
    const n = boundary.getNormalAt(boundary.length * t);
    ...
    const idx = Math.max(0, Math.min(pathIndex, children.length - 1));
    const path = children[idx];
    if (path.length === 0) return new paper.Point(0, 1);
    const n = path.getNormalAt(path.length * t);
    return n || new paper.Point(0, 1);
  }
