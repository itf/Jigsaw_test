 ...
 */
export function getExactSegment(path: paper.PathItem, offset1: number, offset2: number, reverse: boolean = false): paper.Path {
  let sourcePath: paper.Path;
  if (path instanceof paper.CompoundPath) {
    const firstChild = path.children.find(c => c instanceof paper.Path) as paper.Path;
    if (!firstChild) return new paper.Path({ insert: false });
    sourcePath = firstChild;
  } else {
    sourcePath = path as paper.Path;
  }

  const len = sourcePath.length;
  if (len === 0) return new paper.Path({ insert: false });

  const o1 = ((offset1 % len) + len) % len;
  const o2 = ((offset2 % len) + len) % len;

  let clone = sourcePath.clone({ insert: false });
  
  try {
    if (clone.closed) {
      const remainder = clone.splitAt(o1);
      if (remainder) remainder.remove();
      clone.closed = false;
    } else {
      if (o1 > 0.0001 && o1 < len - 0.0001) {
        const remainder = clone.splitAt(o1);
        clone.remove();
        if (remainder) {
          clone = remainder;
        } else {
          clone = new paper.Path({ insert: false });
        }
      } else if (o1 >= len - 0.0001) {
        clone.remove();
        clone = new paper.Path({ insert: false });
        clone.add(sourcePath.lastSegment.point);
      }
    }

    const newLen = clone.length;
    let newO2 = (o2 - o1 + len) % len;
    if (newO2 < 0.0001 && Math.abs(offset1 - offset2) > 0.0001) {
      newO2 = len;
    }

    if (newO2 > 0.0001 && newO2 < clone.length - 0.0001) {
      const secondPart = clone.splitAt(newO2);
      if (secondPart) secondPart.remove();
    }
  } catch (e) {
    console.error('Error in getExactSegment:', e);
  }
