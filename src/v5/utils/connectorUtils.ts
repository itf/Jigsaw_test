...
// Legacy whimsy type used only in connector path generation (custom SVG heads)
type LegacyWhimsy = { id: string; svgData: string };
...
/**
 * @deprecated V3 only. V5 uses the graph model; neighbor lookup is not needed.
 */
export function findNeighborPiece(
  _areas: Record<string, unknown>,
  _currentPieceId: string,
  _point: paper.Point,
  _normal: paper.Point
): string | null {
  return null;
...
export function generateConnectorPath(
  ...
  // If extrusion is negative, the connector is "inward".
  // To keep the geometry consistent (p1 on left, p2 on right relative to extrusion),
  // we swap t1 and t2 when extrusion is negative.
  const isNegative = extrusion < 0;
  const t1 = isNegative ? midT + halfWidthT : midT - halfWidthT;
  const t2 = isNegative ? midT - halfWidthT : midT + halfWidthT;
  
  const getWrappedPoint = (t: number) => {
    const wrappedT = (t + 100) % 1; // Handle negative t
    if (sourcePath.length === 0) return new paper.Point(0, 0);
    const pt = sourcePath.getPointAt(sourcePath.length * wrappedT);
    return pt || new paper.Point(0, 0);
  };
  const getWrappedNormal = (t: number) => {
    const wrappedT = (t + 100) % 1;
    if (sourcePath.length === 0) return new paper.Point(0, 1);
    const n = sourcePath.getNormalAt(sourcePath.length * wrappedT);
    return n || new paper.Point(0, 1);
  };
  ...
  const midPoint = sourcePath.length > 0 ? sourcePath.getPointAt(sourcePath.length * midT) : null;
  const midNormal = sourcePath.length > 0 ? sourcePath.getNormalAt(sourcePath.length * midT) : null;
  ...
  if (useEquidistantHeadPoint) {
    ...
    pt1Head = headPath.length > 0 ? (headPath.getPointAt(hOffset1) || new paper.Point(0, 0)) : new paper.Point(0, 0);
    pt2Head = headPath.length > 0 ? (headPath.getPointAt(hOffset2) || new paper.Point(0, 0)) : new paper.Point(0, 0);
