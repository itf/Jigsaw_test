...
import { 
  Area, AreaType, Operation, OperationType, PuzzleState, Point, Connector, Whimsy, NeckShape,
  Node, Edge, Face
} from '../types';
import { resetPaperProject } from '../../v3/utils/paperUtils';
import { generateGridPoints, generateHexGridPoints, generateRandomPoints } from '../../v3/utils/gridUtils';
import { getWhimsyTemplatePathData, WhimsyTemplateId, DEFAULT_WHIMSIES } from '../../v3/utils/whimsyGallery';
import { validateAndCleanState } from '../../v3/utils/puzzleValidation';
import { findNeighborPiece, generateConnectorPath } from '../../v3/utils/connectorUtils';
import { useStamps } from '../../v3/hooks/useStamps';
import { getNeighborDepth } from '../../v3/utils/relativeUtils';
...
export function usePuzzleEngineV5(): {
  ...
  splitFace: (faceId: string, node1Id: string, node2Id: string, pathData: string) => void;
  deleteEdge: (edgeId: string) => void;
  bakeConnectors: () => void;
  splitFaceAtPoints: (faceId: string, ptA: Point, ptB: Point) => void;
  splitPiece: (id: string, ptA: Point, ptB: Point) => void;
} {
  ...
  const [useGraphMode, setUseGraphMode] = useState(false);
  ...
  const createRoot = useCallback((w: number, h: number, shape: 'RECT' | 'CIRCLE' | 'HEX' = 'RECT') => {
    ...
    if (shape === 'CIRCLE') {
      boundary = new paper.Path.Circle({ center: [w / 2, h / 2], radius: Math.min(w, h) / 2, insert: false });
    } else if (shape === 'HEX') {
      boundary = new paper.Path.RegularPolygon({ center: [w / 2, h / 2], sides: 6, radius: Math.min(w, h) / 2, insert: false });
    } else {
      boundary = new paper.Path.Rectangle({ point: [0, 0], size: [w, h], insert: false });
    }

    // Ensure CCW orientation so that interior is on the left and normals point OUT
    ...
    const newFaces: Record<string, Face> = {};

    // For a simple rectangle/polygon, we can extract nodes and edges
    if (boundary instanceof paper.Path) {
      const faceId = 'face-root';
      const edgeIds: string[] = [];
      
      boundary.segments.forEach((seg, i) => {
        const nodeId = `node-${i}`;
        newNodes[nodeId] = {
          id: nodeId,
      ...
      boundary.curves.forEach((curve, i) => {
        const fromId = `node-${i}`;
        const toId = `node-${(i + 1) % boundary.segments.length}`;
        const edgeId = `edge-${i}`;
        ...
        newNodes[fromId].incidentEdges.push(edgeId);
        newNodes[toId].incidentEdges.push(edgeId);
        edgeIds.push(edgeId);
        edgePath.remove();
      });
      ...
      newFaces[faceId] = {
        id: faceId,
        edges: edgeIds.map(id => ({ id, reversed: false })),
        color: '#f8fafc',
        groupMemberships: []
      };
    }
  ...
  const subdivideGrid = useCallback((params: { parentId: string, pattern: string, rows: number, cols: number, count: number, jitter: number }) => {
    ...
    if (useGraphMode) {
      ...
      if (pattern === 'GRID') {
        manager.subdivideGrid(parentId, rows, cols);
      } else if (pattern === 'HEX') {
        manager.subdivideHex(parentId, rows, cols, jitter);
      } else if (pattern === 'RANDOM') {
        manager.subdivideRandom(parentId, count, jitter);
      }
      setNodes({ ...manager.getNodes() });
      setEdges({ ...manager.getEdges() });
      setFaces({ ...manager.getFaces() });
      return;
    }
    ...
    setAreas(prev => {
      ...
      if (pattern === 'GRID' && jitter === 0) {
        ...
        for (let r = 0; r < rows; r++) {
          for (let c = 0; c < cols; c++) {
            const rect = new paper.Path.Rectangle({ point: [bounds.x + c * dx, bounds.y + r * dy], size: [dx, dy], insert: false });
        ...
        if (points.length === 0) { parentPath.remove(); return prev; }
      ...
      parentPath.remove();
      nextAreas[parentId] = { ...parent, type: AreaType.GROUP, children: childIds };
      return nextAreas;
    });
  }, [width, height, useGraphMode, nodes, edges, faces]);

  const splitFace = useCallback((faceId: string, nodeAId: string, nodeBId: string, pathData: string) => {
    const manager = new GraphManager(nodes, edges, faces);
    manager.splitFace(faceId, nodeAId, nodeBId, pathData);
    setNodes({ ...manager.getNodes() });
    setEdges({ ...manager.getEdges() });
    setFaces({ ...manager.getFaces() });
  }, [nodes, edges, faces]);
  ...
  const bakeConnectors = useCallback(() => {
    const manager = new GraphManager(nodes, edges, faces);
    manager.bakeConnectors(Object.values(connectors), whimsies);
    setNodes({ ...manager.getNodes() });
    setEdges({ ...manager.getEdges() });
    setFaces({ ...manager.getFaces() });
    setConnectors({});
  }, [nodes, edges, faces, connectors, whimsies]);

  const splitFaceAtPoints = useCallback((faceId: string, ptA: Point, ptB: Point) => {
    const manager = new GraphManager(nodes, edges, faces);
    manager.splitFaceAtPoints(faceId, ptA, ptB);
    setNodes({ ...manager.getNodes() });
    setEdges({ ...manager.getEdges() });
    setFaces({ ...manager.getFaces() });
  }, [nodes, edges, faces]);

  const splitPiece = useCallback((id: string, ptA: Point, ptB: Point) => {
    setAreas(prev => {
      const area = prev[id];
      if (!area || (area.type !== AreaType.PIECE && area.type !== AreaType.STAMP)) return prev;
      resetPaperProject(width, height);
      const boundary = area.boundary.clone();
      const line = new paper.Path.Line(new paper.Point(ptA.x, ptA.y), new paper.Point(ptB.x, ptB.y));
      const intersections = boundary.getIntersections(line);
      ...
      const mid = intersections[0].point.add(intersections[intersections.length-1].point).divide(2);
      const normal = intersections[intersections.length-1].point.subtract(intersections[0].point).rotate(90).normalize(10000);
      
      const poly1 = new paper.Path();
      poly1.add(intersections[0].point);
      poly1.add(intersections[intersections.length-1].point);
      poly1.add(intersections[intersections.length-1].point.add(normal));
      poly1.add(intersections[0].point.add(normal));
      poly1.closePath();
      
      const part1 = boundary.intersect(poly1);
      const part2 = boundary.subtract(poly1);
      ...
      const id1 = `${id}-split-1-${Math.random().toString(36).slice(2, 6)}`;
      const id2 = `${id}-split-2-${Math.random().toString(36).slice(2, 6)}`;
      
      const neighborColors2 = getNeighborColors(nextAreas, part2, [id, id1]);
      
      nextAreas[id1] = { ...area, id: id1, boundary: part1, color: area.color, seedPoint: part1.bounds.center };
      nextAreas[id2] = { ...area, id: id2, boundary: part2, color: pickUniqueColor(neighborColors2), seedPoint: part2.bounds.center };
      
      area.groupMemberships.forEach(groupId => {
        if (nextAreas[groupId]) {
          nextAreas[groupId] = { ...nextAreas[groupId], children: nextAreas[groupId].children.filter(cid => cid !== id).concat(id1, id2) };
        }
      });
  ...
  const mergePieces = useCallback((ids: string[]) => {
    ...
    if (useGraphMode) {
      // Find edges between selected faces and delete them
      const manager = new GraphManager(nodes, edges, faces);
      const facesToMerge = ids.map(id => faces[id]).filter(Boolean);
      
      // Find shared edges
      const edgeCounts: Record<string, number> = {};
      facesToMerge.forEach(f => {
        f.edges.forEach(eId => {
          edgeCounts[eId] = (edgeCounts[eId] || 0) + 1;
        });
      });

      const sharedEdges = Object.keys(edgeCounts).filter(eId => edgeCounts[eId] > 1);
      sharedEdges.forEach(eId => manager.deleteEdge(eId));
  ...
  const addWhimsy = useCallback((params: { templateId: string, center: Point, scale: number, rotationDeg: number, color?: string }) => {
    ...
    setAreas(prev => {
      ...
      const whimsyPath = new paper.CompoundPath({ pathData: stem, insert: false });
      ...
      whimsyPath.translate(new paper.Point(center.x - (wb.x + wb.width / 2), center.y - (wb.y + wb.height / 2)));
      whimsyPath.reorient(true, true);
      
      let nextAreas = { ...prev };
      let filledPath: paper.PathItem = whimsyPath.clone({ insert: false });
      if (filledPath instanceof paper.CompoundPath) {
        ...
        if (filled) { filledPath.remove(); filledPath = filled; }
      }
      ...
      pieceIds.forEach(id => {
        ...
        if (piecePath.intersects(filledPath) || piecePath.contains(filledPath.bounds.center) || filledPath.contains(piecePath.bounds.center)) {
          ...
          if (outside.isEmpty()) { delete nextAreas[id]; } else { nextAreas[id] = { ...area, boundary: outside }; }
          if (!inside.isEmpty()) {
            const holePieceId = `hole-${id}-${Math.random().toString(36).slice(2, 6)}`;
            nextAreas[holePieceId] = { ...area, id: holePieceId, boundary: inside, color: area.color, seedPoint: inside.bounds.center };
            area.groupMemberships.forEach(groupId => {
              if (nextAreas[groupId]) {
                nextAreas[groupId] = { ...nextAreas[groupId], children: [...nextAreas[groupId].children, holePieceId] };
              }
            });
          } else { inside.remove(); }
        }
      ...
      nextAreas[whimsyId] = { id: whimsyId, groupMemberships: [], type: AreaType.PIECE, children: [], boundary: whimsyPath.clone({ insert: false }), color: color || pickUniqueColor(neighborColors), seedPoint: center };
      whimsyPath.remove();
        ...
        if (path instanceof paper.Path) {
          ...
          if (isRect) {
            for (let i = 0; i < 4; i++) {
              ...
              if (Math.abs(dot) > 0.01) { results.push(`Piece ${id} is not perfectly rectangular (dot product: ${dot.toFixed(4)})`); }
            }
          } else { results.push(`Piece ${id} does not have 4 segments (count: ${path.segments.length})`); }
        }
      }
    });
    if (results.length === 0) { alert('Grid Validation: All pieces are perfectly rectangular!'); } else { alert(`Grid Validation Failures: ${results.length} issues found. Check console.`); }
  }, [areas]);

  const cleanPuzzle = useCallback(() => { setAreas(prev => validateAndCleanState(prev)); }, []);

  const loadState = useCallback((state: PuzzleState) => {
    setAreas(state.areas);
    setConnectors(state.connectors);
    setWhimsies(prev => {
      const defaultWhimsies = DEFAULT_WHIMSIES.map(w => ({ id: w.id, name: w.name, svgData: getWhimsyTemplatePathData(w.id as WhimsyTemplateId), category: w.category }));
    ...
    if (state.faces) setFaces(state.faces);
    setUseGraphMode(state.useGraphMode || false);
  }, []);
  ...
  const addConnector = useCallback((connector: Omit<Connector, 'id'>) => {
    ...
    setConnectors(prev => ({ ...prev, [id]: { ...connector, id, jitterSeed: (Math.random() * 0xffffffff) >>> 0 } }));
  }, []);
  ...
  const updateConnector = useCallback((id: string, updates: Partial<Connector>) => {
    setConnectors(prev => {
      ...
      const hasChanges = Object.entries(updates).some(([key, value]) => current[key as keyof Connector] !== value);
  ...
  const removeConnector = useCallback((id: string) => {
    setConnectors(prev => { const next = { ...prev }; delete next[id]; return next; });
  }, []);

  const addWhimsyToLibrary = useCallback((whimsy: Whimsy) => { setWhimsies(prev => [...prev, whimsy]); }, []);
  const removeWhimsyFromLibrary = useCallback((id: string) => { setWhimsies(prev => prev.filter(w => w.id !== id)); }, []);

  const generateMassConnectors = useCallback((params: MassConnectorParams): Record<string, Connector> => {
    const { pieceIds, widthRange, widthRelative, extrusionRange, extrusionRelative, positionRange, headTemplateIds, headScaleRange, headScaleRelative, useActualAreaForScale, headRotationRange, jitterRange, neckShapes } = params;
    const generated: Record<string, Connector> = {};
    pieceIds.forEach(id => {
      const area = areas[id];
      if (!area || area.type !== AreaType.PIECE) return;
      const boundary = area.boundary;
      const children = boundary instanceof paper.CompoundPath ? (boundary.children.filter(c => c instanceof paper.Path) as paper.Path[]) : [boundary as paper.Path];
      children.forEach((path, pathIndex) => {
        ...
        neighborSegments.forEach((segments, neighborId) => {
          ...
          segments.forEach(seg => {
            ...
            let sharedLen = 0;
            if (widthRelative) { sharedLen = path.length / 4; }
            ...
            if (widthRelative) { widthPx = widthPx * sharedLen; }
            ...
            if (headScaleRelative) {
              ...
              if (metrics) {
                ...
                if (baseArea > 0) { headScale = Math.sqrt(((headScale / 100) * neighborAreaVal) / (144 * baseArea)); }
              }
            }
            ...
            generated[connectorId] = { id: connectorId, pieceId: id, pathIndex, midT, widthPx, extrusion, headTemplateId, headScale, headRotationDeg: rand(headRotationRange), jitter: finalJitter, jitterSeed: (Math.random() * 0xffffffff) >>> 0, useEquidistantHeadPoint: true, neckShape };
          });
        });
      });
    });
  ...
  const resolveConnectorConflicts = useCallback(() => {
    setConnectors(prev => {
      ...
      connectorList.forEach(c => {
        ...
        if (!area) return;
        try {
          const result = generateConnectorPath(area.boundary, c.pathIndex, c.midT, c.widthPx, c.extrusion, c.headTemplateId, c.headScale, c.headRotationDeg, c.useEquidistantHeadPoint, whimsies, c.jitter, c.jitterSeed || 0);
          paths[c.id] = new paper.CompoundPath({ pathData: result.pathData, insert: false });
        } catch (e) { console.error('Error calculating path for conflict resolution:', e); }
      });
      for (let i = 0; i < connectorList.length; i++) {
        ...
        for (let j = i + 1; j < connectorList.length; j++) {
          ...
          if (!p2) continue;
          if (p1.intersects(p2)) {
            if (c1.id < c2.id) { next[c1.id].disabled = true; } else { next[c2.id].disabled = true; }
          }
        }
      }
  ...
  const puzzleState = useMemo(() => ({
    areas, connectors, whimsies, rootAreaId: rootAreaId || '', width, height,
    nodes, edges, faces, useGraphMode
  }), [areas, connectors, whimsies, rootAreaId, width, height, nodes, edges, faces, useGraphMode]);

  return {
    puzzleState, createRoot, subdivideGrid, mergePieces, addWhimsy, addConnector, updateConnector, removeConnector, addWhimsyToLibrary, removeWhimsyFromLibrary, addMassConnectors, generateMassConnectors, commitPreviewConnectors, resolveConnectorConflicts, validateGrid, cleanPuzzle, loadState, reset, stamps: stampOps,
    splitFace, deleteEdge, bakeConnectors, splitFaceAtPoints, splitPiece
  };
}
